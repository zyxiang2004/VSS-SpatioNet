...
class TransformerBranchWithSS2D(nn.Module):
    def __init__(self, in_planes, out_planes, groups=8, kernel_size=56, stride=1, bias=False, d_state=16):
        super(TransformerBranchWithSS2D, self).__init__()
        self.ss2d = SS2D(d_model=in_planes, dropout=0, d_state=d_state)

        self.conv1x1 = nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride)
        self.norm = nn.LayerNorm(out_planes)
        self.stride = stride

    def forward(self, x):
        ss2d_out = self.ss2d(x)
        ss2d_out = self.conv1x1(ss2d_out)
        ss2d_out = self.norm(ss2d_out)
        return ss2d_out
...
class FusionBlock_res(nn.Module):
    def __init__(self, channels, img_size, index, d_state=16):
        super(FusionBlock_res, self).__init__()
        self.ss2d = SS2D(d_model=channels, dropout=0, d_state=d_state)
    ...
    def forward(self, x_ir, x_vi):
        a_cat_ir = self.ss2d(x_ir)
        a_cat_vi = self.ss2d(x_vi)
        a_cat = torch.cat([a_cat_ir, a_cat_vi], 1)
